...
;;; Create the base for refueling/water supplies ->STATION to reload
...
to setup-units
   create-units fire-units-num [
      set shape "fire-unit"
      set color blue
      set water initial-water ;;; each ground unit can carry a limited amount of water
...
to execute-behaviour
   ...
   ;;;so that it is able to  effectively extinguish fires
   if detect-fire [put-out-fire stop] ;;;  sensor[action stop]
   if need-water [reload-water stop]
...
to-report detect-obstacle
  ifelse any? fires in-cone 2 60 or any? other units in-cone 2 60 or [pcolor] of patch-ahead 1 = blue
   [report true]
   [report false]
end
...
to put-out-fire
      ...
      ;;;One unit of water is required to extinguish the fire on one tree.
      set water water - 1
      set saved-trees saved-trees + 1
end

;;;Reload when Water supplies of the unit are exhausted,
;;;then the unit must return to the station .
to reload-water
  move-back-station
  service-unit
end
...
;; neighbors -> Reports an agentset containing the 8 surrounding patches (neighbors) or 4 surrounding patches (neighbors4).
;; return back to the station(base)
to move-back-station
  move-to patch 0 0
end
